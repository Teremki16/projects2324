...
function createDownLoadLinkXML(order){
    ...
    console.log(parser.parseFromString(xmlStr, "alication/xml").getElementsByTagName("order")[0]);

    let download = document.createElement("a")
    download.setAttribute("href", `data:text/plain;charset=utf-8,${encodeURIComponent(xmlStr)}`)
